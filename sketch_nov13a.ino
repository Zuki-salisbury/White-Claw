...
void setup() {
  ...
  pwm.setPWM(4, 0, 250);  // Wrist
  pwm.setPWM(5, 0, clawPosition);  // Claw
  pwm.setPWM(1, 0, shoulderPosition1);  // Shoulder Part 1
  pwm.setPWM(0, 0, shoulderPosition2);  // Shoulder Part 2
  ...
  if (Serial.available() > 0) {
    ...
    if (input == '3') {
      if (isWristAtMax) {
        ...
        wristPosition = servoMax - 200;  // Move wrist to max position
      }
    ...
    if (input == '5') {
      if (isShoulderFlipped) {
        // shoulderPosition1 = 307;
        // shoulderPosition2 = 307;
        shoulderPosition1 = 200;
        shoulderPosition2 = 400;
      } else {
        shoulderPosition1 = 120;
        shoulderPosition2 = 600;
      }
      pwm.setPWM(1, 0, shoulderPosition1);
      pwm.setPWM(0, 0, shoulderPosition2);
